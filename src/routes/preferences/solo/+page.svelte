...
<div class="min-h-screen bg-[#1A1A32] flex flex-col items-center" 
  ...
  <div class="w-full flex flex-col items-center flex-grow justify-end relative">
    ...
    <div class="absolute bottom-[calc(20%-90px)] z-10 w-full px-4" in:fade={{ duration: 400, delay: 200 }}>
      <div class="max-w-[319px] mx-auto flex gap-2">
        <button 
          class="w-[155.5px] h-[56px] bg-[#D0EEF9] text-[#2A37A3] rounded-lg flex items-center justify-center gap-2 font-semibold"
          on:click={() => selectUserType('famille')}
        >
          <img src="/iconesPerso/all-icons-bl.png" alt="Icône famille" class="w-6 h-6" />
    ...
    <div class="w-screen" in:fade={{ duration: 400, delay: 300 }}>
      <img src="/mascottes/mascotteSolo.png" alt="Mascotte" class="w-full h-auto" />
    </div>
  </div>
</div> 