# Create a new file for editing a member
<script lang="ts">
  ...
  onMount(() => {
    ...
    if (member) {
      ...
      isReducedMobility = member.isReducedMobility;
      selectedInterests = member.interests || [];
    } else {
...
<svelte:head>
  <title>Modifier un membre | Moov</title>
  <style>
    nav {
...
<div class="min-h-screen bg-[#3643BA] flex flex-col items-center" 
  ...
  <div class="w-[327px] mt-6" in:fade={{ duration: 400 }}>
    <h1 class="text-white font-bold text-2xl leading-8">Modifier un membre de votre famille</h1>
  </div>
